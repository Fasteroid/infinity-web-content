const tips = [
    ...
    "When troubleshooting, never be afraid to `print()` the related variables.",
    ...
    "`setName()` can be used to add realtime debug information to an E2's name UI.",
    ...
    "Accidentally abandoned your E2 code?&nbsp; Check this server's special `'autosave'` folder for it!",
    "Optimize your code wherever possible.",
    "Arrays and tables can store multiple values.",
    ...
    "Taking even roots of negative values is 100% forbidden!",
    ...
    "`holoParent()` your holos once instead of calling `holoPos()` every tick. &nbspIt saves performance AND looks better!",
    ...
    "Starfall is a better choice if you want to make each player see something different.",
    "Starfall can run both client and serverside. &nbsp;E2 only runs serverside.",
    "E2 can be easier to work with, but Starfall technically has more potential.",
    "Hitting serverside CPU quotas? &nbsp;Try using Starfall to compute on your client!",
    "The only thing E2 really has on Starfall is its superior documentation and easy wiremod interfacing.",
    "`positive()` is to `vector` as `abs()` is to `number`",
    "Don't forget—arrays here start at index `1` and NOT index `0`.",
    "`foreach(K:string,V:type = Table)` is to E2 as `for K, V in pairs(Table)` is to Starfall.",
